...
# Specify number of time steps (# of slices - 1), regions, tolerance in M optimisation, and convergence threshold
n_tsteps = 3
lim = 225
tol = 1e-4
conv_per = 0.001
...
# Import fake data
N = np.load('{}{}{}'.format('Fake Data ', lim, '/N.npy'))
M_est = np.load('{}{}{}'.format('Fake Data ', lim, '/M_est.npy'))
M_true = np.load('{}{}{}'.format('Fake Data ', lim, '/M_true.npy'))
dist = np.load('{}{}{}'.format('Fake Data ', lim, '/d.npy'))
K_val = np.load('{}{}{}'.format('Fake Data ', lim, '/k.npy'))
...
#Compute the average change in N over time for each slice (used in M initialisation)
fac_list = []
...
while NAE_lim == False:
    ...
    while conv == False:
        ...
        newvals = scipy.optimize.fmin_l_bfgs_b(app_log_lik, XYZ, args=(log_mu, log_Npi, log_Npi_inv), fprime=app_jac, bounds=bnds)
        ...
        newvals2 = scipy.optimize.fmin_l_bfgs_b(f_s_b, sbeta, args=(XYZ,), approx_grad=True, bounds=bnds)
    ...
    for i in range(n_tsteps):
        for j in range(lim):
            for k in range(lim):
                ...
                denom += M_true[i][j][k]
    NAE_Alg_2 = num/denom
    NAE_Alg_1 = num_Nathan/denom
    print('Current NAE = ', NAE_Alg_2)
        ...
        for j in range(lim):
            for k in range(lim):
                if k != j:
                    ...
                    num_Nathan += np.abs(M_true[i][j][k] - M_est[i][j][k])
                    denom += M_true[i][j][k]
    off_NAE_Alg_2 = num/denom
    off_NAE_Alg_1 = num_Nathan/denom
...
print("NAE (alternative algorithm) = ", NAE_Alg_2)
print("NAE (primary algorithm) = ", NAE_Alg_1)
print("off diagonal NAE (alternative algorithm) = ", off_NAE_Alg_2)
print("off diagonal NAE (primary algorithm) = ", off_NAE_Alg_1)
print('Number of regions = ', lim)
print('Number of timesteps = ', n_tsteps)
print('beta = ', sbeta[-1])
print('ftol, Lambda, convergence criterion = ', [tol, lmbda, conv_per])

# Add lines here to save outputs as desired or print
