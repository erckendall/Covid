...
while conv == False:
    ...
    newvals = scipy.optimize.fmin_l_bfgs_b(f_s_b, sbeta,  approx_grad=True, bounds=bnds, epsilon=1e-12)
...
def orig_penalty(Marr):
    ...
    np.array(M_trunc)
    np.array(M_trunc_t)
    first = np.sum(abs(np.sum(N_trunc - np.sum(M_trunc, axis=2), axis=1))**2)
    second = np.sum(abs(np.sum(N_trunc_p1 - np.sum(M_trunc_t, axis=2), axis=1))**2)
    return first + second
