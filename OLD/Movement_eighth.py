...
while conv == False:
    ...
    newvals = scipy.optimize.fmin_l_bfgs_b(f_s_b, sbeta,  approx_grad=True, bounds=bnds, epsilon=1e-6)
    ### Need to automatically set the value of epsilon depending on error
    try:
    ...
    for i in range(comb.shape[0]):
        bnds.append((0, None))
    newvals = scipy.optimize.minimize(fun=loglik, x0=comb, method="SLSQP", bounds=bnds)
    XYZ = newvals.x
    current = newvals.fun
...
for i in range(Marr.shape[0]):
    bnds.append((0, None))
Mvals = scipy.optimize.minimize(fun=loglik_ex, x0=Marr, method="SLSQP", bounds=bnds)
Mfinal = Mvals.x
try:
...
### Original Minimise this to maximise objective function - now all combined
def to_min(comb):
    return -(loglik(comb) - (lmbda / 2) * penalty(comb))

### Original (now combined with loglik) Surely the below penalty must include a sum over i even though not written in the paper
def penalty(comb):
    for t in range(n_tsteps-2):
        for i in range(len(names)):
            Y_trunc[t][i] = comb[Y_plus + t * dy[1] + i]
...
### Original penalty function for comparison
def penalty_og(comb):
    tot = 0
    for t in range(n_tsteps - 2):
        first = 0
...
### Original final penalty function for comparison
def orig_penalty_og(Marr):
    tot = 0
    for t in range(n_tsteps - 2):
        first = 0
        second = 0
...
### Original version of exact log likelihood
def loglik_ex_orig(Marr):
    first, second, third = (0, 0, 0)
    for t in range(n_tsteps - 2):
        for i in range(len(names)):
            first += np.log(1 - pi_f[i]) * Marr[t * dM[2] * dM[1] + i * dM[2] + i]
    for t in range(n_tsteps - 2):
        for i in range(len(names)):
            for j in range(len(names)):
                if j != i:
                    sum = np.log(pi_f[i]) + np.log(s[j]) - beta * dist[i, j]
                    s_sum = 0
    ...
    for t in range(n_tsteps - 2):
        for i in range(len(names)):
            for j in range(len(names)):
                if j != i:
                    third += (Marr[t * dM[2] * dM[1] + i * dM[2] + j] - Marr[
                        t * dM[2] * dM[1] + i * dM[2] + j] * np.log(Marr[t * dM[2] * dM[1] + i * dM[2] + j]))
    return (first + second + third)

### Original penalty - now combined into loglik_ex. Surely the below penalty must include a sum over i even though not written in the paper
def orig_penalty(Marr):
    ...
    for t in range(n_tsteps-2):
        for i in range(len(names)):
            for j in range(len(names)):
                M_trunc[t][i][j] = int(Marr[t * dM[2] * dM[1] + i * dM[2] + j])
                M_trunc_t[t][j][i] = int(Marr[t * dM[2] * dM[1] + i * dM[2] + j])
    np.array(M_trunc)
    np.array(M_trunc_t)
    fst = np.sum(abs(np.sum(N_trunc - np.sum(M_trunc, axis=2), axis=1))**2)
    snd = np.sum(abs(np.sum(N_trunc_p1 - np.sum(M_trunc_t, axis=2), axis=1))**2)
...
### minimisation of loglik_ex - now combined with penalty. Minimise this to maximise objective function for final M calculation
