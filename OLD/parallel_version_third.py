...
#Check prange improvement at high lim
#fastmath seems to actually slow things down
...
# opt = {'maxiter': 100, 'ftol': 3.0} ### Note that implementing this seems to severely change M output (tol was 2.3)
# Mvals = scipy.optimize.minimize(fun=loglik_ex, x0=Marr, method="SLSQP", bounds=bnds, constraints=cons, options=opt, tol=3.0)
# Mfinal = Mvals.x
# try:
...
Mvals = scipy.optimize.fmin_l_bfgs_b(loglik_ex, np.float64(Marr), approx_grad=True, bounds=bnds, epsilon=1e-10, maxiter=1000, factr=1e7, pgtol=4)
Mfinal = Mvals[0]
print '---------------------------'
try:
...
# https://intellipaat.com/community/17253/scipy-optimize-fminlbfgsb-returns-abnormalterminationinlnsrch
